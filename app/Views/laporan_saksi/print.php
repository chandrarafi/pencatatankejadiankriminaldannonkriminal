...
<head>
    ...
    <title>Laporan Data Saksi - Polsek Lunang Silaut</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    <div class="container">
        ...
        <div class="report-info">
            <table>
                <tr>
                    <td class="label">Total Saksi</td>
                    <td>: <?= $totalSaksi ?> orang</td>
                </tr>
                <tr>
        ...
        <?php if (!empty($saksiData)): ?>
            <table class="data-table">
                ...
                <tbody>
                    <?php foreach ($saksiData as $index => $saksi): ?>
                        <tr>
                            <td style="text-align: center;"><?= $index + 1 ?></td>
                            <td><?= $saksi['nama'] ?></td>
                            <td><?= $saksi['nik'] ?: '-' ?></td>
                            <td style="text-align: center;"><?= substr(ucfirst($saksi['jenis_kelamin']), 0, 1) ?></td>
                            <td style="text-align: center;"><?= $saksi['umur'] ?></td>
                            <td><?= $saksi['alamat'] ?: '-' ?></td>
                            <td style="text-align: center;">
                                <?php
                                $jenisSaksiLabel = match ($saksi['jenis_saksi']) {
                                    'saksi_mata' => 'Mata',
                                    'saksi_telinga' => 'Telinga',
                                    'saksi_korban' => 'Korban',
                                    'saksi_ahli' => 'Ahli',
                                    default => ucfirst(str_replace('_', ' ', $saksi['jenis_saksi']))
                                };
        ...
        <?php else: ?>
            <div class="no-data">
                <h4>Tidak ada data saksi yang tersedia.</h4>
                <p>Belum ada saksi yang terdaftar dalam sistem.</p>
            </div>
        <?php endif; ?>
